...
export class EnhancedElevenLabsTTS {
  ...
  private async processQueue(): Promise<void> {
    ...
    try {
      ...
      // Don't log interruptions as errors, they're expected behavior
  ...
  private async playAudio(audioData: ArrayBuffer): Promise<void> {
    ...
    if (this.audioSource) {
      try {
        ...
        console.log("Could not stop previous audio source:", e)
      }
      this.audioSource = null
    }

    // If interruption is already in progress, don't start new audio
    if (this.interruptionInProgress || this.abortController?.signal.aborted) {
      throw new Error("Speech interrupted")
    }
            ...
            const checkInterruption = () => {
              if (this.abortController?.signal.aborted || this.interruptionInProgress) {
                try {
                  ...
                  console.log("Could not stop audio source during interruption check:", e)
                }
                if (this.audioSource === source) {
                  this.audioSource = null
                }
  ...
  interrupt(): void {
    // If not speaking, nothing to interrupt
    if (!this.isSpeaking) {
      return
    }
    ...
    if (this.audioSource) {
      try {
        ...
        console.log("Could not stop audio source:", e)
      }
      this.audioSource = null
    }
